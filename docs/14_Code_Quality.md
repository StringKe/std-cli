...
`mise run test` 会设置 `STD_TEST_MODE=1`。该模式下 release/debug binary 即使被测试间接调用，也会阻断 `open`、`osascript` 和其他 external runner。默认测试严禁唤起 Terminal、App、1Password、WeChat、menu bar resident 或全局热键。
