...
安装版 `std-launcher` 的真实 GUI 全局热键序列仍需本轮显式 desktop opt-in 重跑。历史临时安装目录输出不得列入已验证证据；Launcher UI 仍需要按 docs/18-21 完整视觉审计。

```text
STD_ALLOW_DESKTOP_AUTOMATION=1 .std-cli/install-check/bin/std-launcher --gui-hotkey-smoke Alt+Space 7000
status=未完成，本轮尚未重跑
```

完成前必须覆盖默认隐藏、真实全局快捷键、关闭只隐藏、隐藏后 hotkey runtime 仍注册、二次唤起。
...
安装版 `std` 的真实 `/Applications/WeChat.app` 多语言搜索仍需本轮安装版重跑。当前代码和 fixture 已覆盖 `微信`、`weixin`、`wechat`、macOS `Info.plist`、localized `InfoPlist.strings`、URL scheme、派生别名路径。

```text
.std-cli/install-check/bin/std search 微信
status=未完成，本轮尚未重跑

.std-cli/install-check/bin/std search weixin
status=未完成，本轮尚未重跑

.std-cli/install-check/bin/std search wechat
status=未完成，本轮尚未重跑
```

完成前必须用安装版输出重新证明 `微信`、`weixin`、`wechat` 指向同一个 WeChat app action。
...
当前 Studio 截图证据：未完成。旧 `/tmp` 截图不得列入已验证证据；下一次验收必须来自本轮 `STD_ALLOW_UI_PREVIEW=1 mise run ui-capture-matrix` 的 manifest，并覆盖 light / dark、Dashboard、Workflow、Analysis、Plugin、Operations、Memory、History、Settings、panes 等场景。
...
```text
.std-cli/install-check/bin/std --help
Commands:
