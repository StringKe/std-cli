...
#[test]
fn panel_surface_width_matches_docs_width() {
...
#[test]
fn panel_rect_matches_panel_sized_native_host() {
    let mut state = LauncherState::new();
    state.update_query("index");
...
#[test]
fn panel_rect_matches_panel_sized_window() {
    let mut state = LauncherState::new();
    state.update_query("index");
    let available = egui::Rect::from_min_size(egui::pos2(0.0, 0.0), window_inner_size(&state));
    let rect = panel_rect(available, &state);
...
#[test]
fn native_host_is_panel_sized_without_visible_carrier() {
    let mut state = LauncherState::new();
    state.update_query("index");
    let available = egui::Rect::from_min_size(egui::pos2(0.0, 0.0), window_inner_size(&state));
    let rect = panel_rect(available, &state);
...
#[test]
fn panel_rect_owns_entire_native_host() {
    let mut state = LauncherState::new();
    ...
    assert_eq!(rect.min, egui::pos2(0.0, 0.0));
    assert_eq!(rect.width(), PANEL_WIDTH);
    assert_eq!(available.width() - rect.width(), 0.0);
    assert_eq!(available.height() - rect.height(), 0.0);
    assert!(panel_is_only_visible_surface(&state));
}
