...
fn launcher_window_smoke_validates_hotkey_window_commands() {
    ...
    assert!(summary.contains(
        "host_positioning=resize-panel-window>outer-position-0.28-monitor-anchor>visible>focus"
    ));
    assert!(summary.contains("native_window=panel-sized-transparent-host"));
    assert!(summary.contains("carrier_background=none"));
}
