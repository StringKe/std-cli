...
#[test]
fn studio_binary_test_mode_blocks_native_window_startup() {
    let output = run_studio(&[]);
...
#[test]
fn studio_binary_test_mode_blocks_ui_preview_window_startup() {
    let output = run_studio(&["--ui-preview", "light", "dashboard", "10"]);
...
fn run_studio(args: &[&str]) -> std::process::Output {
