...
use crate::{ops_index_evidence, ops_plugin_evidence, ops_release_evidence, ops_runbook};
    ...
    pub fn load() -> Self {
        ...
        Self {
            ...
            release: OpsGate {
                ...
                status: ops_release_evidence::release_status(&release_dir),
                evidence: release_dir.display().to_string(),
                result: ops_release_evidence::release_result(&release_dir),
                detail: "manifest, binaries, app bundles, docs, examples, quality".to_string(),
                artifact: release_dir
                    .join("release-manifest.json")
                    .display()
                    .to_string(),
                output: ops_release_evidence::release_output(&release_dir),
            },
            install: OpsGate {
                title: "Install",
                ...
                status: ops_release_evidence::install_status(&install_prefix),
                evidence: install_prefix.display().to_string(),
                result: ops_release_evidence::install_result(&install_prefix),
                detail: "installed binaries, app bundles, storage directories".to_string(),
                artifact: install_prefix.display().to_string(),
                output: ops_release_evidence::install_output(&install_prefix),
            },
            plugin: OpsGate {
                title: "Plugin",
