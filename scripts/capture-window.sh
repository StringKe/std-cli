#!/usr/bin/env bash
set -euo pipefail

if [ "$#" -ne 4 ]; then
  echo "usage: scripts/capture-window.sh <process-pid> <process-name> <window-title-fragment> <output-png>" >&2
  exit 2
fi

process_pid="$1"
process_name="$2"
title_fragment="$3"
output_png="$4"

if [ "$(uname -s)" != "Darwin" ]; then
  echo "capture-window requires macOS" >&2
  exit 2
fi

if [ "${STD_ALLOW_UI_PREVIEW:-}" != "1" ]; then
  echo "capture-window SKIP reason=STD_ALLOW_UI_PREVIEW=1 required" >&2
  exit 2
fi

attempts="${STD_CAPTURE_ATTEMPTS:-30}"
attempt=1
while [ "$attempt" -le "$attempts" ]; do
  if /Applications/Xcode.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/swift \
    scripts/cg-capture-window.swift "$process_pid" "$process_name" "$title_fragment" "$output_png" 2>/dev/null; then
    test -s "$output_png"
    exit 0
  fi
  /bin/sleep 0.5
  attempt=$((attempt + 1))
done

echo "window not found: $process_pid / $process_name / $title_fragment" >&2
exit 1
